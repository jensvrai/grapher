...
export default class Linker {
    ...
    if (!this.isResolver()) {
      if (!this.linkConfig.collection) {
        throw new Meteor.Error(
          'invalid-config',
          `For the link ${this.linkName} you did not provide a collection. Collection is mandatory for non-resolver links.`
        );
      }
  ...
  /**
     ...
     */
  _extendSchema() {
    if (this.isVirtual() || this.isResolver() || 1===1) {
      return;
    }

    if (
      this.mainCollection.simpleSchema &&
      this.mainCollection.simpleSchema()
    ) {
      ...
      if (
        !this.mainCollection
          .simpleSchema()
          ._schemaKeys.includes(this.linkConfig.field)
      ) {
    ...
    if (Meteor.isServer) {
      ...
      if (this.linkConfig.index) {
        ...
        if (this.linkConfig.unique) {
          if (this.isMany()) {
            throw new Meteor.Error(
              'You cannot set unique property on a multi field.'
            );
          }
