...
export default class Link {
  ...
  _validateIds(ids) {
    ...
    const validIds = this.linkedCollection
      .find(
        {
          _id: { $in: ids },
        },
    ...
    if (validIds.length != ids.length) {
      throw new Meteor.Error(
        'not-found',
        `You tried to create links with non-existing id(s) inside "${this.linkedCollection._name}": ${_.difference(ids, validIds).join(', ')}`
      );
    }
  }
